...
from src.config.config import DIRECTORIES, MAX_FILES_PER_FOLDER, SFTP_CONFIG
...
async def download_thumbnail(session: aiohttp.ClientSession, video_id: str, thumbnail_url: str, save_path: str, is_docker: bool = False) -> bool:
...
async def download_batch_thumbnails(videos: list, base_dir: str, folder_name: str, is_docker: bool = False) -> Dict[str, Any]:
    ...
    async with aiohttp.ClientSession() as session:
        ...
        for video in videos:
            ...
            save_path = base_dir + f"/{video_id}_thumbnail.jpg"
            # Remove project root path for local storage
            if not is_docker:
                relative_path = save_path.replace("D:/OSINT/youtube-crawl/", "")
            else:
...
def download_video_thumbnails(videos: list) -> Dict[str, Any]:
    ...
    if is_docker:
        ...
        base_dir = SFTP_CONFIG["remote_base_path"] + "/videos/thumbnails"
    ...
    for i in range(0, len(videos), 100):
        ...
        results = asyncio.run(download_batch_thumbnails(batch, base_dir, base_dir, is_docker))
...
def download_video_thumbnails_auto(videos: list) -> Dict[str, Any]:
    """Download thumbnails for videos in batches of 100."""
    count_success = 0
    updated_videos = []
    today_str = datetime.now().strftime('%Y-%m-%d')
